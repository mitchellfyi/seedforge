...
export async function getStepsByProjectId({
  projectId,
}: {
  projectId: string;
}) {
...
export async function getStepById({ id }: { id: string }) {
  try {
    const [found] = await db.select().from(step).where(eq(step.id, id));
    return found ?? null;
  } catch (_error) {
    throw new ChatSDKError("bad_request:database", "Failed to get step by id");
  }
}
...
// =============================================================================
// Seedforge: Need to Know Queries
// =============================================================================

export async function createNeedToKnows(
  items: Array<{
    projectId: string;
    stepId?: string;
    title: string;
...
export async function getNeedToKnowsByProjectId({
  projectId,
}: {
  projectId: string;
}) {
  try {
    return await db
      .select()
      .from(needToKnow)
      .where(eq(needToKnow.projectId, projectId));
  } catch (_error) {
    throw new ChatSDKError(
      "bad_request:database",
...
// =============================================================================
// Seedforge: Learner Profile Queries
// =============================================================================
...
export async function getLearnerProfile({ userId }: { userId: string }) {
  try {
...
export async function updateLearnerProfile({
  ...
  initialScaffoldingLevel?: string;
  avatarPreset?: string;
  totalGp?: number;
  level?: number;
...
// =============================================================================
// Seedforge: Garden Plant Queries
// =============================================================================

export async function createGardenPlant(data: {
  userId: string;
  projectId: string;
  plantType: string;
  domain?: string;
...
export async function getGardenPlantsByUserId({ userId }: { userId: string }) {
  try {
    return await db
      .select()
      .from(gardenPlant)
      .where(eq(gardenPlant.userId, userId))
      .orderBy(asc(gardenPlant.createdAt));
  } catch (_error) {
    throw new ChatSDKError(
      "bad_request:database",
